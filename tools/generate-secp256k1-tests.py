...
def print_validation_test_case(
    f, num_cases, filter_pk, filter_msg, filter_sig, expect: str
):
    ...
    for sk in sks:
        ...
        sha.update(msg)
        f.write(
            f"secp256k1_verify 0x{bytes(filter_pk(pk.serialize())).hex()} 0x{filter_msg(sha.digest()).hex()} 0x{bytes(filter_sig(sk.ecdsa_serialize_compact(sig))).hex()}"
        )
...
with open(p, "w+") as f:
    ...
    print_validation_test_case(
        f, SIZE, lambda pk: pk, lambda msg: msg, lambda sig: sig, "0"
    )
