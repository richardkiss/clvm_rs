...
///
/// **Deprecated**: Use `intern()` instead, which provides atoms/pairs lists.
...
pub fn intern_node(allocator: &Allocator, node: NodePtr) -> Result<(Allocator, NodePtr)> {
    ...
    Ok((tree.allocator, tree.root))
}

/// Creates an interned version of a node with visit counts.
///
...
pub fn create_interned_node(
    allocator: &Allocator,
    node: NodePtr,
) -> Result<(Allocator, NodePtr, Vec<(NodePtr, usize)>)> {
    let tree = intern(allocator, node)?;

    // Compute visit counts by traversing the tree (for backward compatibility)
    let mut counter: HashMap<NodePtr, usize> = HashMap::new();
    let mut count_stack = vec![tree.root];

    while let Some(current) = count_stack.pop() {
        counter.entry(current).and_modify(|c| *c += 1).or_insert(1);
        if let SExp::Pair(left, right) = tree.allocator.sexp(current) {
            count_stack.push(left);
            count_stack.push(right);
        }
    }

    let counts: Vec<(NodePtr, usize)> = counter.into_iter().collect();
    Ok((tree.allocator, tree.root, counts))
}

/// Compute stats given lists of atom and pair NodePtrs from an interned allocator.
///
/// **Deprecated**: Use `InternedTree::stats()` instead.
#[deprecated(since = "2.0.0", note = "Use InternedTree::stats() instead")]
