...
use crate::python_dialect::PythonDialect;
...
#[pyfunction]
pub fn run_serialized_chia_program_dialect(
    py: Python,
    program: &[u8],
    args: &[u8],
    max_cost: Cost,

    custom_op_callback: Bound<PyAny>,
    ...
    let r: Response = (|| -> PyResult<Response> {
        let program = node_from_bytes(&mut allocator, program)?;
        let args = node_from_bytes(&mut allocator, args)?;
        let dialect = PythonDialect::new(custom_op_callback, custom_op_list);
...
fn clvm_rs(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(run_serialized_chia_program_dialect, m)?)?;
